 ...
 * 
 */
public class DiscardThreeAndOneHandler implements IDiscardHandler {
	...
	public DiscardThreeAndOneHandler(MessageInfo.Builder msgInfo, Room room,
	...
	private Builder checkCards() {
		if (CollectionUtils.isEmpty(cardIds) || cardIds.size() != 4) {
			msgInfo = commonBiz.setMessageInfo(
					MessageConstants.DISCARD_CARDS_ISNOT_THREE_AND_ONE_TYPE,
					MessageConstants.DISCARD_CARDS_ISNOT_THREE_AND_ONE_MSG);
			return msgInfo;
		}
		return null;
	}
	...
	private Builder checkCardsRule(List<Integer> cardsPoint,
		...
		if (map.size() != 2 || !(map.containsValue(3) && map.containsValue(1))) {
			msgInfo = commonBiz.setMessageInfo(
					MessageConstants.DISCARD_CARDS_ISNOT_THREE_AND_ONE_TYPE,
					MessageConstants.DISCARD_CARDS_ISNOT_THREE_AND_ONE_MSG);
			return msgInfo;
