 ...
 * 
 */
public class DiscardFourAndTwoHandler implements IDiscardHandler {
	...
	public DiscardFourAndTwoHandler(MessageInfo.Builder msgInfo, Room room,
	...
	private Builder checkCardsRule(List<Integer> cardsPoint,
		...
		if (map.size() == 3 && map.containsValue(4) && map.containsValue(2)) {// 四带两对
			return null;
		} else if (map.size() == 3 && map.containsValue(4)
				&& map.containsValue(1)) {// 四带两张单牌
			return null;
		} else {
			msgInfo = commonBiz.setMessageInfo(
					MessageConstants.DISCARD_CARDS_ISNOT_THREE_AND_TWO_TYPE,
