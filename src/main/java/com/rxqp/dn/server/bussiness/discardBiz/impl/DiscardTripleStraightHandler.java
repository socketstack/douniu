 ...
 * 
 */
public class DiscardTripleStraightHandler implements IDiscardHandler {
	...
	public DiscardTripleStraightHandler(MessageInfo.Builder msgInfo, Room room,
	...
	public Builder discard() {
		...
		for (Integer preCardId : preCardIds) {
			if (preCardId == 54 || preCardId == 53) {// 大小王
				cardsPoint.add(preCardId);
			} else {
				cardsPoint.add(preCardId % 13);
			}
		}
	...
	private Builder checkCards() {
		if (CollectionUtils.isEmpty(cardIds) || cardIds.size() < 6
				|| cardIds.size() % MODE != 0) {// 至少2对以上,每对3张
			msgInfo = commonBiz.setMessageInfo(
					MessageConstants.DISCARD_CARDS_ISNOT_TRIPLE_STRAIGHT_TYPE,
					MessageConstants.DISCARD_CARDS_ISNOT_TRIPLE_STRAIGHT_MSG);
			return msgInfo;
	...
	private Builder checkCardsRule(List<Integer> cardsPoint) {
		for (int i = 0; i < cardsPoint.size() / MODE; i = i + MODE) {
			if (!(cardsPoint.get(i).equals(cardsPoint.get(i + 1)) && cardsPoint
					.get(i).equals(cardsPoint.get(i + 2)))) {// 相邻三张要相等
				msgInfo = commonBiz
						.setMessageInfo(
								MessageConstants.DISCARD_CARDS_ISNOT_TRIPLE_STRAIGHT_TYPE,
								MessageConstants.DISCARD_CARDS_ISNOT_TRIPLE_STRAIGHT_MSG);
