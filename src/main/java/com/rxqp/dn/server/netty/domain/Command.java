package com.rxqp.dn.server.netty.domain;
...
import com.rxqp.dn.server.utils.StringUtils;
...
public class Command {
		...
		case HTTP:
			...
			if ((this.commandData != null) && (this.commandData.size() >= 1)) {
				this.id = Integer.valueOf(
						((String) this.commandData.get(0)).trim()).intValue();
	...
	int readInt() {
		...
		case HTTP:
		case WEBSOCKET_TEXT:
			return Integer.parseInt((String) this.commandData
					.get(this.readIndex++));
		case SOCKET:
		case WEBSOCKET_BINARY:
	...
	short readShort() {
		...
		case HTTP:
		case WEBSOCKET_TEXT:
			return Short.parseShort((String) this.commandData
					.get(this.readIndex++));
		case SOCKET:
		case WEBSOCKET_BINARY:
	...
	long readLong() {
		...
		case HTTP:
		case WEBSOCKET_TEXT:
			return Long.parseLong((String) this.commandData
					.get(this.readIndex++));
		case SOCKET:
		case WEBSOCKET_BINARY:
	...
	float readFloat() {
		...
		case HTTP:
		case WEBSOCKET_TEXT:
			return Float.parseFloat((String) this.commandData
					.get(this.readIndex++));
		case SOCKET:
		case WEBSOCKET_BINARY:
